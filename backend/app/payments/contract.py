...
SUPPORTED_PAYMENT_PROVIDERS = {"wompi", "mock", "mercado_pago", "stripe"}
...
class LocalPaymentGatewayAdapter:
    ...
    def validate_integration(
        ...
        *,
        ...
        if _normalize_status(integration_status) != "active":
            return
        payment_link_ttl_minutes(config)
        if self.provider != "mock" and not str(credentials_raw or "").strip():
            raise HTTPException(
                status_code=http_status.HTTP_422_UNPROCESSABLE_CONTENT,
    ...
    def create_payment_link(
        ...
        *,
        ...
        return PaymentLinkResult(
            url=f"https://payments.example.test/pay/{reference}",
            reference=reference,
            link_id=None,
...
def get_payment_adapter(provider: str | None) -> PaymentGatewayAdapter:
    ...
    if normalized in {"mock", "mercado_pago", "stripe"}:
        return LocalPaymentGatewayAdapter(normalized)
    return LocalPaymentGatewayAdapter("mock")
...
def record_payment_transaction(
    ...
    *,
    ...
    if payment_link_id:
        payment_metadata["link_id"] = str(payment_link_id).strip() or None
